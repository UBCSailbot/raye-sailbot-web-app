...
class Connection:
    ...
    def Subscribe(self, uri, callback=None):
        """Subscribes to the specified uri in the network table and polls the 
        data in the request uri 
    ...
    def manageSocket(self):
        """Polls the network table and performs the function provided in subscribe.
        *DISCLAIMER: Not the same manageSocket in Connection.cpp
        """
        def getReply():
            reply_message = self.socket.recv()
            reply_node = Reply_pb2.Reply()
